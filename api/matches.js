export default async function handler(request, response) {
  ...
  const endpointType = urlObj.searchParams.get("endpoint") || "matches";

  let externalUrl = "https://api.football-data.org/v4/competitions/WC/matches?season=2026";

  if (endpointType === "scorers") {
    externalUrl = "https://api.football-data.org/v4/competitions/WC/scorers?season=2026";
  } else if (endpointType === "standings" || endpointType === "clasification") {
    externalUrl = "https://api.football-data.org/v4/competitions/WC/standings?season=2026";
  }
